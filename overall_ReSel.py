...
class scirex_bert(nn.Module):
    ...
    def forward(self, doc, tup_idx, result, train_flag):
        predictions = []
        labels = []
        logits = []
        loss = 0
        ...
        if self.args.dataset == 'scirex':
            ...
            adjacent_graph = doc.coref_graph + doc.cooccur_graph
            num_aspects = 2
        elif self.args.dataset == 'tdms':
            adjacent_graph = doc.coref_graph + doc.cooccur_graph + doc.align_graph
            num_aspects = 3
        ...
        pred = result
        if type(pred).__name__ == 'list':
            if self.partial == True:
                entity_ids_set = []
                for p in pred:
                    for i in range(len(doc.entity_ids_label[p])):
                        if self.args.dataset == 'scirex'  or self.args.dataset == 'tdms':
                            if doc.entity_ids_label[p][i] != -1 and doc.entity_ids_label[p][i] not in entity_ids_set and doc.entity_types[doc.entity_ids_label[p][i]] == 'score':
                                entity_ids_set.append(doc.entity_ids_label[p][i])
                        else:
                            if doc.entity_ids_label[p][i] != -1 and doc.entity_ids_label[p][i] not in entity_ids_set:
                                entity_ids_set.append(doc.entity_ids_label[p][i])
                entity_ids_set.sort()
            else:
        ...
        if self.args.dataset == 'scirex' or self.args.dataset == 'tdms':
            if torch.argmax(score) in label_list:
                acc += 1
        elif self.args.dataset == 'pubmed':
            if ''.join(doc.entities[entity_ids_set[torch.argmax(score)]]) == doc.gt_tuples[tup_idx][-1]:
                acc += 1
        logits.append(score)
        ...
        if train_flag:
            return score, label_list, loss, acc
        else:
            return score, label_list, acc, prediction
